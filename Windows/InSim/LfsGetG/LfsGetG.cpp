...
#define SERVER_PORT 4123		// SimToolsがこのポートだったので揃えた
...
----------------------------------------------------------------------------*/
#define BUF_SIZE 1024		// 256でいいのかもしれないが念のため
void SimDataReceiveAndTx()
{
	...
	while (1)
	{
		...
		else if (recv_size == sizeof(pack))	// 受信した？
		{
			...
			// 確認用
			AngVx = pack.AngVel.x;	// 角速度[rad/s]
			AngVy = pack.AngVel.y;
			AngVz = pack.AngVel.z;
			Vx = pack.Vel.x;		// 速度[m/s](ワールド座標)
			Vy = pack.Vel.y;
			Vz = pack.Vel.z;
			//Ax = Ay = Az = 0;	// デバッグ用
